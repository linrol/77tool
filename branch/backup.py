...
class Backup(Common):
    def __init__(self, source, target, clear, projects):
        super().__init__(utils, projects, False)
        self.source = source
        self.target = target
        self.clear = clear

    def execute(self):
        backup_projects = list(self.projects.keys())
        ...
        if len(backup_projects) < 1:
            print("分支【{}】不存在需要备份的工程".format(self.source))
            sys.exit(1)
        ...
        print("基于【{}】创建【{}】分支的工程【{}】成功".format(self.source, self.target, created_projects))
...
# python3 backup.py sprint20220922 stage-global global,platform,init-data
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("ERROR: 输入参数错误, 正确的参数为：<source_branch>[.clear] <target_branch> <end|module|projects>")
        sys.exit(1)
    else:
        clear_source = ".clear" in sys.argv[1]
        source_branch = sys.argv[1].replace(".clear", "")
        target_branch = sys.argv[2]
        project_list = sys.argv[3].split(",")
        Backup(source_branch, target_branch, clear_source, project_list).execute()
