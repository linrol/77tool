...
class Task:
    ...
    def check_create_branch(self, source_branch, target_branch, project_list):
        ...
        need_project_list = list(filter(
            lambda name: self.get_project(name).getBranch(
                target_branch) is None, project_list))
        if len(need_project_list) < 1:
            raise Exception("ERROR: \n" + "工程【{}】目标分支【{}】已存在!!!".format(
                ",".join(project_list), target_branch))
    ...
    def build_create_branch_task(self, send_template_card, req_user_id,
        ...
        project_list):
        try:
            _, _, project_names = self.check_create_branch(source, target,
                                                           project_list)
            ...
            task_content = "{}@{}".format(task_code, project_names)
