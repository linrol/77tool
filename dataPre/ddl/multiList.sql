...
COMMENT ON COLUMN "baseapp_list_columns_schema_status_field"."customized_fields" IS '自定义属性';
COMMENT ON TABLE "baseapp_list_columns_schema_status_field" IS '列表模型状态字段';
CREATE UNIQUE INDEX "idx_baseapp_list_columns_schema_status_field_xak1" ON "baseapp_list_columns_schema_status_field" USING btree("list_column_schema_id" varchar_pattern_ops ASC NULLS FIRST,"field_name" ASC NULLS FIRST) WHERE is_deleted = false;
COMMENT ON INDEX "idx_baseapp_list_columns_schema_status_field_xak1" IS '状态字段不可重复。';